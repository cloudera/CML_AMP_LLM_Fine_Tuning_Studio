...
def handle_database_import():
    with st.container():
        import_hf_tab, upload_tab = st.tabs(["**Import Huggingface Dataset**", "**Import CSV Dataset**"])
...
def display_csv_import_tab():
    ...
    import_dataset = c3.button("Import", type="primary", use_container_width=True, key="csv_import")
    ...
    st.info("""
        ...
        **How to Prepare Data for Import:**
        ...
        - Right now only "single split" datasets are supported. (i.e., you cannot import a train vs. test split).
