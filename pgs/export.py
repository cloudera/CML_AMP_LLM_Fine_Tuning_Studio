...
with st.container(border=True):
    with st.container():
        ...
        with col1:
            registered_model_name = st.text_input("Registered Model Name", placeholder="human-friendly name in model registry", key="registered_model_name")

        with col2:
            current_models = get_state().models
            model_idx = st.selectbox("Base Model", range(len(current_models)), format_func=lambda x: current_models[x].name, index=None)
    ...
    with st.container():
        ...
        with col1:
            model_description = st.text_input("Model Description", placeholder="description of the model", key="model_description")

        with col2:
            model_adapters = get_state().adapters
            if model_idx is not None:
                current_model: ModelMetadata = current_models[model_idx]
                model_adapters: List[AdapterMetadata] = list(filter(lambda x: x.model_id == current_model.id, model_adapters))
            else:
                model_adapters = []

            adapter_idx = st.selectbox("Choose an Adapter", range(len(model_adapters)), format_func=lambda x: model_adapters[x].name, index=None)

    # Start job button
    button_enabled = model_idx is not None and adapter_idx is not None and registered_model_name != ""
    start_job_button = st.button("Register Model to CML Model Registry", type="primary", use_container_width=True, disabled=not button_enabled)
    ...
    if start_job_button:
        try:
            with st.spinner("Generating an MLFlow model pipeline..."):
                ...
                pipeline = fetch_pipeline(model_name=model.huggingface_model_name, adapter_name=adapter_location_or_name)
            ...
            st.success("Exported Model. Please go to **CML Model Registry** to view your model!", icon=":material/check:")
