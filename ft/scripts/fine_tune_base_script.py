...
parser.add_argument("--axolotl_train_config_id", help="ID of the axolotl training arguments in FT Studio's config store.", default=None)
parser.add_argument("--finetuning_framework_type", help="Finetuning Frameowkr to be used for Model training.", default=FinetuningFrameworkType.FINETUNING_FRAMEWORK_TYPE_PROPRIETARY_SOLUTION)
...
if args.finetuning_framework_type == str(FinetuningFrameworkType.FINETUNING_FRAMEWORK_TYPE_AXOLOTL):
    # Get the configurations.
    axolotl_train_config_dict = yaml.safe_load(
        fts.GetConfig(
            GetConfigRequest(
                id=args.axolotl_train_config_id
            )
        ).config.config
    )
...
elif args.finetuning_framework_type == str(FinetuningFrameworkType.FINETUNING_FRAMEWORK_TYPE_PROPRIETARY_SOLUTION):
