...
class Dataloader():
    ...
    def fetch_evaluation_dataset(dataset_id: str, total_examples: int = 50, client: FineTuningStudioClient = None):
        ...
        dataset = load_dataset(dataset.huggingface_name, cache_dir=get_unique_cache_dir())
        eval_df = pd.DataFrame(dataset["train"])
        eval_column_name, template_function = fetch_eval_column_name_and_merge_function(dataset.huggingface_name)
