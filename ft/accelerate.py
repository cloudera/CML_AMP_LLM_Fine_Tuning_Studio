...
def launch_distributed_script(tune_script):
    try:
        # Total number of workers to use (including the main session)
    ...
    if NUM_GPU_WORKERS == 1:
        text_file = open(
            "amp_3_job_fine_tune/distributed_peft_scripts/common/accelerate_configs/accelerate_single_config.yaml.tmpl")
    elif NUM_GPU_WORKERS > 1:
        text_file = open(
            "amp_3_job_fine_tune/distributed_peft_scripts/common/accelerate_configs/accelerate_multi_config.yaml.tmpl")
    else:
        sys.exit(
    ...
    for i in range(NUM_GPU_WORKERS):
        ...
        config_file = config_file.substitute(
            MACHINE_RANK=i,
            MAIN_SESSION_IP=MASTER_IP,
            NUM_MACHINES=NUM_GPU_WORKERS,
            NUM_PROCESSES=NUM_GPU_WORKERS)
